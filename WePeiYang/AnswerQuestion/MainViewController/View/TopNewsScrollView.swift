...
//    }
    ...
    fileprivate func addMainView() {
        ...
        scrollView.contentOffset = CGPoint(x: self.frame.width, y: 0)   // 初始页设为6页中的第二页
        scrollView.delegate = self
        scrollView.isPagingEnabled = true
        scrollView.showsHorizontalScrollIndicator = false
        for index in 0 ..< imageArray.count {
            let imageView = UIImageView()
            imageView.frame = CGRect(x: CGFloat(index)*self.frame.width, y: 0, width:
                scrollView.frame.width, height: scrollView.frame.height)
            imageView.image = UIImage(color: UIColor.red)
